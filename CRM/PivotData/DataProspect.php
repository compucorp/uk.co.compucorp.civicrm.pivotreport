...
class CRM_PivotData_DataProspect extends CRM_PivotData_DataCase {
  ...
  protected function getEntityApiParams(array $inputParams) {
    $params = [
      ...
      'api.ProspectConverted.get' => ['prospect_case_id' => '$value.id'],
      ...
      if (!empty($inputRow['api.ProspectConverted.get']['values'][0])) {
        ...
        if ((int) $inputRow['api.ProspectConverted.get']['values'][0]['payment_type_id'] === CRM_Prospect_BAO_ProspectConverted::PAYMENT_TYPE_CONTRIBUTION) {
          $contribution = civicrm_api3('Contribution', 'get', [
              'sequential' => 1,
              'id' => $paymentEntityId,
              'return' => ['id', 'financial_type_id', 'contribution_status', 'total_amount', 'receive_date', 'receipt_date'],
              'options' => [
                'limit' => 1,
              ],
            ]
          ...
          $pledge = civicrm_api3('Pledge', 'get', [
              'sequential' => 1,
              'id' => $paymentEntityId,
              'return' => ['id', 'pledge_status', 'pledge_amount', 'pledge_total_paid', 'pledge_create_date', 'pledge_start_date', 'pledge_end_date', 'pledge_financial_type'],
              'options' => [
                'limit' => 1,
              ],
            ]
  ...
  protected function getFields() {
    if (empty($this->fields)) {
      ...
      $includeCaseFields = ['case_id', 'case_subject', 'case_status_id', 'case_start_date', 'case_end_date'];
      ...
      $fields['pledge']['pledge_start_date'] = ['title' => ts('Pledge Start Date'), 'type' => CRM_Utils_Type::T_DATE];
      $fields['pledge']['pledge_end_date'] = ['title' => ts('Pledge End Date'), 'type' => CRM_Utils_Type::T_DATE];
